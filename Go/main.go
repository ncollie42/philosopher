...
func main() {
	...
	for x := 1; x < num; x++ {
		...
		phil := &philosopher{id: x, hp: MAX_HP, right: right, left: left, state: none, sem: sem}
		philosophers[x] = phil
		go phil.dine()
		left = right
	}
	phil := &philosopher{id: 0, hp: MAX_HP, right: fork0, left: left, state: none, sem: sem}
	philosophers[0] = phil
